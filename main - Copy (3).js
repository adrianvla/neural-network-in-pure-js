...
class NN {
    ...
    async train(inputs, outputs) {
        var that = this;
        //var cost = await this.cost(await this.save(), inputs, outputs);
        var vectornum = await this.save();
        
        var minim = await approxMinFn(this.cost(vectornum,inputs, outputs),vectornum.length,[inputs,outputs]);
        return costs;
    ...
    async cost(v, inputs, outputs) {
        await this.load(v);
        var costs = 0;
        var that = this;
        for (var j = 0; j < inputs.length; j++) {
            await this.calculate(inputs[j]);
            var cost = 0;
...
async function approxMinFn(fn,argnum,args){
    var testfor = [];
    var testf = [];
    for(var i = 0;i<50;i++){
        var o=[];
        for(var u = 0;u<argnum;u++){
            o[u]=Math.random()*200 * (Math.random() > 0.5 ? 1 : -1);
        }
        testfor.push(fn(await locMinFn(o,fn,args)));
        testf.push(await locMinFn(o,fn,args));
    }
    var m = Math.min(...testfor);

    return testf[testfor.indexOf(m)];
}
async function locMinFn(n_n,fn,args){
    ...
    for (var i = 0; i < vectornum; i++) {
        var goleft=false;
        var pval = await fn(await nudge(turnables, n_n),...args);
        turnables[i] = EPSILON;
        if (pval < await fn(await nudge(turnables, n_n),...args )) {
            goleft=true;
        }
        pval = fn(n_n,...args);
        var flag = true;
        ...
        while(flag){
            ...
            pval = await fn(await nudge(turnables, n_n),...args);
        }
        //n_n=await nudge(turnables,n_n);
    }
    return await nudge(turnables, n_n);
}
async function a(o){
